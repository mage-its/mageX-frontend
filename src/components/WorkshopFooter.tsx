...
import { FaEnvelope, FaFax, FaInstagram, FaLine, FaLinkedin, FaTiktok } from 'react-icons/fa6';
...
function ContactItem({ children, href, icon }: FooterItemProps) {
  return (
    <div className="mb-1 flex items-center text-white opacity-30 font-fredoka font-normal">
      {icon && <span className="mr-2">{icon}</span>}
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="hover:text-orange-primary-2">
          {children}
        </a>
      ) : (
        <span>{children}</span>
...
function MenuItem({ children, href, icon }: FooterItemProps) {
  return (
    <div className="mb-2 flex items-center text-white opacity-30 font-fredoka font-normal">
      {icon && <span className="mr-2">{icon}</span>}
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="hover:text-orange-primary-2">
          {children}
...
function Maps() {
  ...
  return (
    <MapsSection title="MAPS" items={mapItems} />
  );
}
...
function Footer() {
  ...
  return (
    <div className={`bg-[#141414]`}>
      <div className='px-[51px] w-full flex flex-col gap-[30px] py-8'>
        <div className='flex flex-row items-end gap-[80px]'>
          ...
          <span className='w-fit'>
            <h1 className=' w-full bg-vertical-gta bg-clip-text text-transparent text-nowrap font-airstrike font-normal text-7xl text-center'>MAGE X</h1>
            <span className='flex flex-col items-center text-xl text-white text-opacity-60 text-nowrap font-fredoka font-normal text-center'>
