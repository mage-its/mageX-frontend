...
const WorkshopInformation: React.FC<WorkshopInformationProps> = ({ currentWorkshop }) => {
  const scrollRef = useDragScroll();
  
  return (
    <div className="text-light h-full select-none
      ...
      <div 
        ...
        <div>
          <div className="text-justify font-airstrike
                          mobile:text-[26px] ipad:text-[36px] desktop:text-[26px]">{currentWorkshop.title}</div>
          <h2 className="text-gray-5 font-medium w-fit bg-light rounded-[1rem] px-2 py-1 font-fredoka select-none
                         mobile:text-[9px] ipad:text-[18px] desktop:text-[0.7rem]">
              Scroll to see more
          </h2>
        </div>
