...
export const TimelineCard = ({ title, date, order, ...props }: TimelineCardProps) => {
  const [ ref, { width } ] = useMeasure()
  return (
      <div ref={ref} className={cn("min-w-max h-max absolute")} style={order%2 == 1 ?{ left: 140+(300*order)-width/2 , top: "41%" } : { left: 140+(300*order)-width/2, bottom: "48%" }} {...props}>
        {
          order % 2 === 0 ? 
          <>
            <div className="bg-horizontal-gta h-fit w-fit rounded-xl flex items-center mb-4">
              <div className="m-[3px] bg-black h-fit w-fit rounded-[10px] p-7">
                <p className="font-roboto font-bold text-2xl text-light">{date}</p>
                <p className="font-fredoka font-light text-lg text-light max-w-[500px]">{title}</p>
              </div>
            </div>
...
export default function Timeline({ items }: TimelineProps) {
  ...
  return (
    <motion.div drag="x" dragConstraints={{ left: -items.length*300+window.innerWidth-300, right: 0 }} className="w-full h-full relative">
    <div className="absolute top-1/2 -translate-y-1/2 h-fit">
      ...
      <div ref={timlineRef} className="bg-gray-2 h-[5px]" style={{ width: items.length*300}}/>
      <div className=" absolute top-0 -translate-y-1/2 w-8 h-8 rounded-full bg-gray-2" style={{ left: itemsWidth }}/>`
    </div>
